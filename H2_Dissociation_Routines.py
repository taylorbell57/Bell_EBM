...
# Dissociation Fraction from Saha Equation
...
# Derivative of Dissociation Fraction from Saha Equation
...
# Dissociation Fraction from an Approximation to the Saha equation
def dissFracApprox(T, mu=3320.680532597579, std=471.38088012739126):
    return scipy.special.erf((T-mu)/std/np.sqrt(2))/2+0.5

# Derivative of Dissociation Fraction from an Approximation to the Saha Equation
...
def getSahaApproxParams(P = 0.1*const.atm.value):
    ...
    muSaha  = (2/3*b)/scipy.special.lambertw(c*(2/3)*b*P**(-2/3)).real
    stdSaha = (2/3*b)/scipy.special.lambertw(d*(2/3)*b*P**(-2/3)).real-muSaha

    return muSaha, stdSaha

# Heat Capacity of H2 as a Function of Temperature
def cp_H2(T):
    if type(T)!=np.ndarray:
        T = np.array([T])
...
# Derivative of the Heat Capacity of H2 as a Function of Temperature
def delta_cp_H2(T):
